...
\label{eq:edp}
\end{equation}
A more performance-sensitive variant is
\begin{equation}
...
\midrule
Runtime & $T_{\mathrm{app}}$ & Total execution time / makespan \\
Energy & $E_{\mathrm{app}}$ & Total consumed energy \\
EDP & $E_{\mathrm{app}}T_{\mathrm{app}}$ & Balances energy and runtime \\
ED$^2$P & $E_{\mathrm{app}}T_{\mathrm{app}}^2$ & Penalizes slowdown more strongly \\
Slowdown & $T_{\mathrm{app}}/T_{\mathrm{base}} - 1$ & Relative performance loss \\
\bottomrule
\end{tabular}
...
\section{Task Characterization under Frequency Scaling}
\label{sec:theory_task_characterization}

A runtime scheduler cannot rely on expensive offline modelling for every task instance. It needs lightweight task characterization that is accurate enough for scheduling but cheap enough to use online. This section introduces the modelling assumptions used by the thesis.

\subsection{Task-Type-Level Modelling}
\label{subsec:task_type_modeling}

Instead of building a separate model for every task invocation, tasks can be grouped by task type. A task type may be identified by the kernel name, task symbol, operation category, or runtime metadata. For each type $k$, the runtime can estimate execution time and power under a small number of representative frequency configurations:
\begin{align}
\widehat{T}_{k}(\phi) &\approx T_k(\phi), \\
\widehat{P}_{k}(\phi) &\approx P_k(\phi).
\end{align}

This approach is less precise than full per-call modelling, but it is more suitable for a runtime. It also matches task-graph applications where the same operation type appears repeatedly, such as tiled linear algebra or iterative solvers.

\subsection{Frequency Sensitivity Features}
\label{subsec:frequency_sensitivity_features}

A lightweight classifier can use features such as runtime, input size, dependency count, copy time, queue delay, and observed sensitivity to frequency changes. A simple sensitivity score for task type $k$ between high and lower frequency configurations can be defined as
\begin{equation}
\sigma_k = \frac{\widehat{T}_{k}(\phi_{\mathrm{low}}) - \widehat{T}_{k}(\phi_{\mathrm{high}})}{\widehat{T}_{k}(\phi_{\mathrm{high}})}.
\label{eq:frequency_sensitivity}
\end{equation}
A large $\sigma_k$ indicates that the task type is sensitive to frequency reduction, while a small $\sigma_k$ suggests that it may tolerate lower frequency.

\subsection{Runtime Suitability}
\label{subsec:runtime_suitability}

The model must be inexpensive to update. It should avoid complex profiling that interrupts execution or requires many samples per task type. In this thesis, the role of the model is not to predict exact power and runtime for every possible frequency pair. Instead, it provides enough information to distinguish tasks that should remain at high frequency from tasks that are candidates for mid or low frequency under sufficient slack.
...
Exact critical-path computation can be expensive or unavailable in online runtime scheduling. A lightweight proxy can estimate whether a task is likely to be important. The current scheduling design uses features such as task cost, dependency count, and input size. A normalized criticality proxy can be written as
\begin{equation}
\kappa(v) = \mathrm{clip}_{[0,1]}\left(
0.55 c_T(v) + 0.30 c_D(v) + 0.15 c_B(v)
\right),
\label{eq:criticality_proxy}
\end{equation}
where $c_T(v)$ is normalized task cost, $c_D(v)$ is normalized dependency count, and $c_B(v)$ is normalized input-byte volume. The exact weights can be tuned, but the principle is that long, dependency-heavy, data-heavy tasks should be treated more conservatively.
...
LinUCB is a contextual bandit algorithm that assumes the expected reward of an action is approximately linear in the context \cite{li2010linucb}. For each arm $a$, the algorithm maintains a parameter estimate $\theta_a$. Given context $x$, it computes
\begin{equation}
...
\section{Proposal--Commit DVFS Control}
\label{sec:theory_proposal_commit}

The DVFS design in this thesis separates task-level frequency intent from device-level frequency commitment. This avoids making the placement bandit responsible for a very large joint action space of devices and frequencies. Instead, placement is the first layer, and DVFS is the second layer.
...
This separation simplifies learning and attribution. The placement layer reasons about data locality, queue delay, and finish time. The DVFS layer reasons about criticality, slack, backlog, and frequency sensitivity after the device has been selected.

\subsection{Task-Level Proposer}
\label{subsec:task_level_proposer}

The proposer assigns each scheduled task a frequency intent, such as high, mid, or low. The intent is not immediately applied. It is a vote that expresses what the task appears to need under the current context.
...
s(v) = w_{\kappa}\kappa(v) + w_T c_T(v) + w_B b(v)
       - w_W q(v) - w_S s_{\mathrm{slack}}(v) + w_H h(v),
\label{eq:dvfs_proposer_score}
\end{equation}
where $b(v)$ is backlog pressure, $q(v)$ is copy/wait ratio, $s_{\mathrm{slack}}(v)$ is normalized slack, and $h(v)$ indicates a heavy task. Positive terms push the decision toward high frequency, while negative terms indicate that lower frequency may be safe.
...
The committer aggregates task proposals over a device-level window. Only the committer changes the actual committed frequency level. This prevents individual tasks from causing frequent frequency oscillations.
...
\begin{equation}
\Delta T_v(\phi) = \alpha_v T_v \left(1 - q(v)\right)
\left(\frac{f_{g,\mathrm{high}}}{f_{g,\phi}} - 1\right),
\label{eq:estimated_extra_runtime}
\end{equation}
where $q(v)$ is the copy/wait ratio and $\alpha_v$ increases for critical or heavy tasks. The aggregate extra runtime should satisfy a slack-budget condition:
\begin{equation}
...
A proposal--commit system needs guardrails to avoid performance regressions. Typical guardrails include:
\begin{itemize}
    \item force high frequency for windows with high critical mass;
    \item force high frequency when urgent heavy tasks appear;
    \item prevent low frequency under high backlog;
    \item require consecutive eligible windows before entering mid or low;
    \item return to high frequency after recent slowdown is detected.
\end{itemize}

The purpose of these guardrails is not to maximize energy savings for every local window. Instead, they protect the global runtime objective and make DVFS behaviour stable.

\subsection{Algorithmic View}
\label{subsec:algorithmic_view_proposal_commit}

Listing~\ref{lst:proposal_commit_pipeline} summarizes the runtime decision pipeline in simplified form.

\begin{listing}[H]
\caption{Simplified runtime pipeline for placement and proposal--commit DVFS.}
\label{lst:proposal_commit_pipeline}
\begin{minted}[frame=single,fontsize=\small]{text}
for each ready task v:
    estimate candidate cost on each GPU
    d_base = GPU with earliest finish time

    alt = best copy-saving alternative satisfying end-time gate
    d_exec = alt if accepted else d_base

    criticality = estimate_criticality(v)
    proposal = propose_frequency(v, d_exec, criticality)
    add proposal to device-level DVFS window

    committed_level = current committed level of d_exec
    attach committed frequency to task v

    launch task after dependencies and data transfers
    update data residency and device ready time

    if DVFS window is complete:
        aggregate proposal votes and slack budget
        apply guardrails
        commit high, mid, or low for the next window
\end{minted}
\end{listing}
...
At a high level, the runtime attempts to solve the following constrained optimization problem:
\begin{align}
...
\end{align}
where $\pi(v)$ is the placement decision, $\phi(v)$ is the frequency decision, and $J$ may be energy, EDP, ED$^2$P, or another user-selected objective.
...
This chapter presented the theoretical foundation for energy-aware scheduling of multi-GPU task graphs. A CUDASTF application can be viewed as a DAG where tasks, dependencies, data residency, and communication costs are visible to the runtime. GPU DVFS introduces an energy--performance trade-off that depends on task type, frequency sensitivity, criticality, and transition overhead. HEFT-style scheduling provides a strong baseline for performance, but it does not directly optimize energy or data-locality-driven trade-offs. Controlled deviation from HEFT, contextual bandit adaptation, region-level DVFS, and proposal--commit frequency control form the theoretical basis for the framework developed in this thesis.
