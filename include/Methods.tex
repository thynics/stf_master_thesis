% Methods chapter rewritten to match the current implementation.
...
\end{equation}
The implementation uses a deterministic tie-breaker; when two candidates have the same finish time, the lower GPU identifier is selected. We denote the corresponding baseline candidate by
\begin{equation}
\xi_0(t)=\xi(t,g_0(t)).
\end{equation}
...
\end{equation}
Here $C_{\mathrm{ref}}$, $d_{\mathrm{ref}}$, and $B_{\mathrm{ref}}$ are normalization constants. The implementation defaults are
\begin{equation}
...
\end{equation}
Each profile defines a maximum normalized finish-time penalty and a minimum normalized copy gain. The implementation uses separate thresholds for critical and non-critical tasks. With the default parameterization,
\begin{table}[t]
\centering
\caption{Default residual-gate profiles in the current implementation.}
...
Let $\theta_q$ be the criticality threshold used by the implementation. For a selected profile $p$, the active thresholds for task $t$ are
\begin{equation}
\delta(p,t)
=
\left\{
...
\label{eq:gate-copy}
\end{equation}
Equations~\ref{eq:gate-eft} and~\ref{eq:gate-copy} are the locality certificate used by the implementation. They guarantee that every accepted deviation has bounded normalized finish-time loss and sufficient predicted copy reduction.
...
\end{equation}
In the current bandit implementation, $\lambda_Q=0$, so
\begin{equation}
H(t,c)=\rho_K(t,c).
\end{equation}
...
The residual gate profile is selected at the granularity of a scheduling window rather than independently for every task. This design reduces oscillation and lets the scheduler adapt to phases of the task graph. A window contains up to $N_W$ scheduled tasks, where $N_W$ is controlled by \texttt{CUDASTF\_BANDIT\_WINDOW\_SIZE}.
...
The implementation uses a relative reward. For each scheduled task, it also evaluates what reward the default profile would have obtained on the same candidate set. The reward credited to the active profile is therefore
\begin{equation}
r_t^{\mathrm{rel}}
=
...
\begin{algorithm}[t]
\caption{HEFT-relative copy-only placement decision}
...
The DVFS layer is implemented as a conservative proposer and per-GPU window committer. It is separate from the placement bandit: placement chooses the GPU, and DVFS chooses the frequency level to use on that GPU. The stable scheduler alias \texttt{CUDASTF\_SCHEDULE=win\_dvfs} selects the bandit placement path with DVFS enabled. Equivalently, \texttt{CUDASTF\_WIN\_DVFS\_ENABLE=1} enables the same DVFS layer when a compatible bandit placement scheduler is selected.

When DVFS is active, the runtime first checks that the selected placement scheduler supports per-task frequency annotations and that valid graphics-clock levels can be obtained for all GPUs. The implementation defines three abstract levels,
\begin{equation}
\ell\in\{\mathrm{HIGH},\mathrm{MID},\mathrm{LOW}\}.
\end{equation}
The high level is the highest supported graphics clock. The mid level is a lower supported clock chosen by rank, with rank two as the default. The low level is the next lower rank when low-frequency operation is globally allowed. On the default probed path, the memory clock is kept at the highest supported memory clock. If the tuner cannot probe frequency levels, the implementation uses explicit frequency-pair fallbacks only when sufficient high and mid frequencies are provided.
...
\end{equation}
The implementation also records a diagnostic DVFS risk score,
\begin{equation}
z_{\mathrm{dvfs}}(t)
=
\gamma_q q(t)
...
\end{equation}
where the default weights are $0.35$, $0.20$, $0.15$, $0.20$, $0.10$, and $0.15$, respectively. This score is emitted in the task-level DVFS log for diagnostics; the actual proposal is governed by the threshold rules below.
...
For a lower frequency level $\ell$, the implementation estimates the extra runtime as
\begin{equation}
\Delta T_{\ell}(t)
=
...
\State $u_t\gets\Call{MakeProposal}{t,g^{*}}$ \Comment{$u_t\in\{\mathrm{HIGH},\mathrm{MID},\mathrm{LOW}\}$}
...
\State evaluate forced-high, mid-eligible, and low-eligible predicates
\State update consecutive eligibility counters for $\mathrm{MID}$ and $\mathrm{LOW}$
\If{forced-high predicate holds}
    \State $\ell_{g^{*}}\gets\mathrm{HIGH}$
\ElsIf{$\ell_{g^{*}}=\mathrm{LOW}$ and low is not eligible}
    \State $\ell_{g^{*}}\gets\mathrm{HIGH}$
\ElsIf{$\ell_{g^{*}}=\mathrm{MID}$ and mid is not eligible}
    \State $\ell_{g^{*}}\gets\mathrm{HIGH}$
\ElsIf{low is eligible for the required number of consecutive windows}
    \State $\ell_{g^{*}}\gets\mathrm{LOW}$
\ElsIf{mid is eligible for the required number of consecutive windows}
    \State $\ell_{g^{*}}\gets\mathrm{MID}$
\Else
    \State $\ell_{g^{*}}\gets\mathrm{HIGH}$
\EndIf
...
\label{sec:methods-implementation-interface}

The placement scheduler is selected with \texttt{CUDASTF\_SCHEDULE}. Table~\ref{tab:method-interface} summarizes the paper-level configuration interface used to describe the implementation. Experimental code names are intentionally omitted from the method description; the names below refer to the stable scheduler functions rather than to intermediate development aliases.

\begin{table}[t]
\centering
\caption{Scheduler aliases and runtime configuration interface.}
\label{tab:method-interface}
\begin{tabular}{p{0.44\linewidth}|p{0.48\linewidth}}
\hline\hline
Configuration & Meaning \\
\hline
\texttt{CUDASTF\_SCHEDULE=}\newline\texttt{bandit\_scheduler} & Default placement method: HEFT baseline plus copy-only residual gate and window-level LinUCB gate selection. \\
\texttt{CUDASTF\_SCHEDULE=}\newline\texttt{win\_dvfs} & Placement scheduler with the windowed proposal--commit DVFS layer enabled. \\
\texttt{CUDASTF\_SCHEDULE=}\newline\texttt{win\_dvfs\_midrank}$\langle N\rangle$ & \texttt{win\_dvfs} path with a specified mid-frequency rank and the next lower rank used for low frequency. \\
\texttt{CUDASTF\_BANDIT\_}\newline\texttt{WINDOW\_SIZE} & Number of tasks per placement-controller window. \\
\texttt{CUDASTF\_BANDIT\_ALPHA} & LinUCB uncertainty weight. \\
\texttt{CUDASTF\_BANDIT\_LAMBDA} & Ridge regularization used to initialize the LinUCB inverse covariance state. \\
\texttt{CUDASTF\_BANDIT\_REWARD\_ETA} & Deviation-rate penalty applied to the window reward. \\
\texttt{CUDASTF\_BANDIT\_REWARD\_}\newline\texttt{KAPPA\_CRITICAL} and \texttt{KAPPA\_NONCRITICAL} & Penalty weights that convert normalized finish-time loss into the task-level relative reward. \\
\texttt{CUDASTF\_BANDIT\_AGGR\_}\newline\texttt{BIAS\_ALPHA\_SCALE} & Exploration-scale adjustment for the aggressive profile. \\
\texttt{CUDASTF\_BANDIT\_AGGR\_}\newline\texttt{BIAS\_SCORE\_PRIOR} & Additive score prior for the aggressive profile. \\
\texttt{CUDASTF\_BANDIT\_AGGR\_}\newline\texttt{BIAS\_DEFAULT\_MARGIN} & Margin that the default profile must overcome to replace aggressive. \\
\texttt{CUDASTF\_BANDIT\_LOG} and \texttt{CUDASTF\_BANDIT\_PHASE\_LOG} & Emit placement-window and scheduler-phase diagnostics. \\
\texttt{CUDASTF\_WIN\_DVFS\_ENABLE} & Enable the DVFS framework when using a compatible bandit placement scheduler. \\
\texttt{CUDASTF\_WIN\_DVFS\_LOG} & Emit \texttt{WIN\_DVFS\_*} proposal, frequency-map, window, and commit-summary logs. \\
\texttt{CUDASTF\_WIN\_DVFS\_ENABLE\_LOW} & Allow the committer to consider the low-frequency level. \\
\texttt{CUDASTF\_WIN\_DVFS\_MID\_RANK} and \texttt{CUDASTF\_WIN\_DVFS\_LOW\_RANK} & Select the lower graphics-clock ranks used for mid and low levels. \\
\texttt{CUDASTF\_WIN\_DVFS\_WINDOW\_TASKS} & Number of per-GPU task intents accumulated before a DVFS commit is considered. \\
\texttt{CUDASTF\_WIN\_DVFS\_MID\_ENTER\_WINDOWS} and \texttt{LOW\_ENTER\_WINDOWS} & Consecutive eligible windows required before committing mid or low frequency. \\
\texttt{CUDASTF\_WIN\_DVFS\_FORCE\_HIGH\_WINDOWS} and \texttt{LOW\_DISABLE\_WINDOWS} & Recovery intervals used by the slowdown guardrail after unsafe lower-frequency behavior. \\
\hline\hline
\end{tabular}
\end{table}

The implementation logs task-level placement information, residual-gate decisions, window rewards, profile scores, copy statistics, and DVFS proposal and commit summaries. The main placement log prefixes are \texttt{BANDIT\_SCHEDULER\_TASK}, \texttt{BANDIT\_SCHEDULER\_WINDOW\_START}, \texttt{BANDIT\_SCHEDULER\_WINDOW\_END}, and \texttt{BANDIT\_SCHEDULER\_BANDIT\_SCORE}. The DVFS log prefixes are \texttt{WIN\_DVFS\_CONFIG}, \texttt{WIN\_DVFS\_FREQ\_MAP}, \texttt{WIN\_DVFS\_TASK}, \texttt{WIN\_DVFS\_WINDOW}, and \texttt{WIN\_DVFS\_COMMIT\_STATS}. These records are used to connect observed performance differences to concrete scheduling mechanisms: baseline HEFT placement, copy-saving deviations, window-level profile selection, and DVFS frequency commits.
