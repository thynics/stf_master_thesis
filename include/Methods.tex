...
\end{equation}
Here $C_{\mathrm{ref}}$, $d_{\mathrm{ref}}$, and $B_{\mathrm{ref}}$ are normalization constants. Their evaluation defaults are reported in Table~\ref{tab:criticality-hyperparameters}.
...
\end{equation}
where $\beta_C$, $\beta_d$, and $\beta_B$ are tunable non-negative weights. They control the relative contribution of compute cost, dependency fan-in, and input-data volume, respectively, and satisfy $\beta_C+\beta_d+\beta_B=1$ in the default configuration.
...
\centering
\footnotesize
\begin{tabular}{@{}lp{0.54\linewidth}l@{}}
\toprule
Parameter & Role & Evaluation default \\
\midrule
$C_{\mathrm{ref}}$ & Cost normalizer & $500$ scheduler-cost units \\
$K_{\mathrm{ref}}$ & Copy-time normalizer for bandit context & $100$ scheduler-cost units \\
$d_{\mathrm{ref}}$ & Dependency-count normalizer & $16$ \\
$B_{\mathrm{ref}}$ & Input-byte normalizer & $2^{30}$ bytes \\
$\beta_C$ & Task-cost weight & $0.55$ \\
$\beta_d$ & Dependency-count weight & $0.30$ \\
$\beta_B$ & Input-byte weight & $0.15$ \\
$\theta_q$ & Criticality split for residual gates & $0.60$ \\
\bottomrule
\end{tabular}
\caption{Criticality, context-normalization, and residual-gate hyperparameters used in the evaluation.}
\label{tab:criticality-hyperparameters}
\end{table}

\section{Copy-Only HEFT-Relative Residual Gate}
\label{sec:methods-copy-only-gate}

The residual gate is the core placement mechanism. It asks whether any non-baseline GPU can reduce predicted copy time enough to justify a small local delay relative to HEFT. Figure~\ref{fig:methods-placement-motivation} illustrates the motivation. A locally optimal HEFT decision may place the current task on an idle GPU because that device gives the earliest predicted finish time, but the produced data may then be needed by successor tasks on the original GPU. In that case, the local finish-time improvement can create downstream copy traffic. The residual gate therefore treats a non-HEFT placement as acceptable only when the locality evidence is large enough and the predicted finish-time penalty remains bounded.

\figMethodsPlacementMotivation
...
\end{equation}
Each profile defines a maximum normalized finish-time penalty and a minimum normalized copy gain. The gate uses separate thresholds for critical and non-critical tasks. Table~\ref{tab:gate-profiles} shows the default parameterization used in the evaluation.
\begin{table}[t]
\centering
\footnotesize
\begin{tabular}{@{}lcccc@{}}
...
\end{table}
In the default parameterization, $\delta$ denotes the allowed value of $\rho_F$, and $\tau$ denotes the required value of $\rho_K$. The aggressive profile is more permissive because it allows a slightly larger finish-time penalty and requires a smaller copy gain.
...
\right.
\end{equation}
where $p_W$ is the profile selected for the current window and $\mathcal{F}(t,p_W)$ is the feasible set under that profile. Ties are resolved by earlier predicted finish time and then deterministically by GPU identifier. Figure~\ref{fig:methods-residual-flow} summarizes the decision flow.

\figMethodsResidualFlow
...
\overline{q} \\
\phi_{\mathrm{high}} \\
\displaystyle \frac{\log(1+\overline{K}_0)}{\log(1+K_{\mathrm{ref}})} \\
\displaystyle \frac{\log(1+\overline{B}_{\mathrm{in}})}{\log(1+B_{\mathrm{ref}})} \\
\displaystyle \frac{\overline{d}}{d_{\mathrm{ref}}} \\
\displaystyle \frac{\log(1+\overline{C})}{\log(1+C_{\mathrm{ref}})} \\
...
\end{equation}
Only the active profile is updated for the completed window. Figure~\ref{fig:methods-bandit-loop} shows the temporal loop between completed-window statistics, profile selection, execution, and reward update.

\figMethodsBanditLoop

\section{Placement Procedure and State Update}
\label{sec:methods-placement-procedure}

Algorithms~\ref{alg:placement-decision} and~\ref{alg:state-update} summarize the placement procedure and the corresponding scheduler-state update. The first algorithm describes the HEFT-relative decision rule, while the second algorithm describes how the selected placement is made visible to subsequent tasks through the ready-time and residency states. The separation is intentional. Algorithm~\ref{alg:placement-decision} evaluates all candidate GPUs against the same pre-placement state, selects either the HEFT baseline or an accepted residual candidate, and records the information needed for the bandit update. Algorithm~\ref{alg:state-update} is applied only after this decision has been made, so that candidate evaluation is not affected by partially updated residency or ready-time information.

This procedure is executed online for each task that becomes ready in the \CUDASTF{} runtime. The scheduler does not build an offline schedule for the full DAG. Instead, it repeatedly observes the current ready task, estimates the candidate finish time and copy cost on each GPU, applies the active residual gate, and then commits the selected placement to the runtime state. This online structure is important because both device availability and logical-data residency change after every scheduled task. A placement that is attractive before a write-like task may become unattractive immediately afterward if that task invalidates copies on other GPUs.
...
\ForAll{$g\in\mathcal{G}$}
    \State $\xi(t,g)\gets\Call{EvaluateCandidate}{t,g,R,M}$ \Comment{$D,K,S,F,Q$}
\EndFor
...
\If{$\mathcal{F}=\emptyset$}
    \State $g^{*}\gets g_0$
\Else
    \State $g^{*}\gets\arg\max_{g\in\mathcal{F}}H(t,g)$ \Comment{tie: finish time, GPU id}
\EndIf
...
The proposer also detects heavy kernels. A task is marked heavy if its estimated cost exceeds the heavy-task threshold or if its task symbol matches compute-intensive kernels such as GEMM, SYRK, TRSM, POTRF, or POTRS. A heavy task is marked urgent-heavy when it is heavy and has low slack.

The proposal weight accumulated by the window committer is
\begin{equation}
w_{\mathrm{dvfs}}(t)
=
1+\lambda_q^{w}q(t)+\lambda_C^{w}c_{\mathrm{norm}}(t).
\end{equation}
...
\end{equation}
where the weights are listed in Table~\ref{tab:dvfs-proposer-hyperparameters}. These coefficients are fixed across experiments and serve only as diagnostic attribution weights; the actual proposal is governed by the threshold rules below.
...
Parameter & Role & Evaluation default \\
\midrule
$G_{\mathrm{ref}}$ & Task-cost normalizer & $1.0\,\mathrm{ms}$ \\
$\lambda_q^{w}$ & Proposal-weight criticality multiplier & $1.5$ \\
$\lambda_C^{w}$ & Proposal-weight cost multiplier & $1.0$ \\
$\gamma_q$ & Diagnostic risk weight for criticality & $0.35$ \\
$\gamma_C$ & Diagnostic risk weight for task cost & $0.20$ \\
$\gamma_b$ & Diagnostic risk weight for backlog & $0.15$ \\
$\gamma_{\omega}$ & Diagnostic risk weight for copy-wait ratio & $0.20$ \\
$\gamma_s$ & Diagnostic risk weight for slack & $0.10$ \\
$\gamma_h$ & Diagnostic risk weight for heavy tasks & $0.15$ \\
$\theta_{\mathrm{force}}$ & Task-level criticality force-high threshold & $0.90$ \\
$C_{\mathrm{heavy}}^{\mathrm{strict}}$ & Strict heavy-task force-high cost & $1.0\,\mathrm{ms}$ \\
$C_{\mathrm{light}}$ & Maximum task cost for low intent & $0.25\,\mathrm{ms}$ \\
\bottomrule
\end{tabular}
\caption{Task-level DVFS proposer hyperparameters.}
\label{tab:dvfs-proposer-hyperparameters}
\end{table}
...
\begin{equation}
q(t)\geq\theta_{\mathrm{force}},
\qquad
\mathrm{urgentHeavy}(t)=1,
\qquad
C(t)\geq C_{\mathrm{heavy}}^{\mathrm{strict}},
\qquad
b_{\mathrm{norm}}(t,g)\geq b_{\mathrm{force}},
\end{equation}
or if a heavy task is not sufficiently wait-bound and slack-rich. A heavy task is eligible for mid only when
\begin{equation}
\omega(t)\geq\omega_{\mathrm{heavy}}^{\mathrm{mid}},
\qquad
s_{\mathrm{norm}}(t)\geq s_{\mathrm{heavy}}^{\mathrm{mid}},
\qquad
b_{\mathrm{norm}}(t,g)\leq b_{\mathrm{heavy}}^{\mathrm{mid}}.
\end{equation}
For non-forced cases, the mid-level intent is eligible when
\begin{equation}
\omega(t)\geq\omega_{\mathrm{mid}},
\qquad
q(t)\leq q_{\mathrm{mid}},
\qquad
\mathrm{urgentHeavy}(t)=0,
\qquad
b_{\mathrm{norm}}(t,g)\leq b_{\mathrm{mid}}.
\end{equation}
The low-level intent is eligible only under stricter conditions:
\begin{equation}
q(t)\leq q_{\mathrm{low}},
\qquad
s_{\mathrm{norm}}(t)\geq s_{\mathrm{low}},
\qquad
\omega(t)\geq\omega_{\mathrm{low}},
\end{equation}
\begin{equation}
\mathrm{heavy}(t)=0,
\qquad
C(t)\leq C_{\mathrm{light}},
\qquad
b_{\mathrm{norm}}(t,g)\leq b_{\mathrm{low}},
\end{equation}
...
Parameter & Role & Evaluation default \\
\midrule
$b_{\mathrm{force}}$ & Task-level backlog force-high threshold & $0.80$ \\
$\omega_{\mathrm{heavy}}^{\mathrm{mid}}$ & Heavy-task wait ratio for mid intent & $0.60$ \\
$s_{\mathrm{heavy}}^{\mathrm{mid}}$ & Heavy-task slack for mid intent & $0.70$ \\
$b_{\mathrm{heavy}}^{\mathrm{mid}}$ & Heavy-task backlog limit for mid intent & $0.35$ \\
$\omega_{\mathrm{mid}}$ & Wait-ratio threshold for mid intent & $0.25$ \\
$q_{\mathrm{mid}}$ & Criticality limit for mid intent & $0.45$ \\
$b_{\mathrm{mid}}$ & Backlog limit for mid intent & $0.60$ \\
$q_{\mathrm{low}}$ & Criticality limit for low intent & $0.25$ \\
$s_{\mathrm{low}}$ & Slack threshold for low intent & $0.50$ \\
$\omega_{\mathrm{low}}$ & Wait-ratio threshold for low intent & $0.40$ \\
$b_{\mathrm{low}}$ & Backlog limit for low intent & $0.40$ \\
\bottomrule
\end{tabular}
\caption{Task-level DVFS intent thresholds.}
\label{tab:dvfs-intent-thresholds}
\end{table}
...
\alpha_t
=
1+\lambda_q^{\alpha}q(t)+\lambda_h^{\alpha}\mathbf{1}\{\mathrm{heavy}(t)\},
\end{equation}
$T(t)$ is the predicted runtime contribution of the task, and $f_{\ell}$ is the graphics clock of level $\ell$. The default values of $\lambda_q^{\alpha}$ and $\lambda_h^{\alpha}$ are given in Table~\ref{tab:dvfs-committer-hyperparameters}. Equation~\ref{eq:dvfs-extra-runtime} makes the estimated slowdown larger for compute-bound, critical, or heavy tasks and smaller for copy-wait-bound tasks.

A commit is considered when the per-GPU window reaches the configured number of intents $N_{\mathrm{dvfs}}$. Let $V_{\mathrm{HIGH}}$, $V_{\mathrm{MID}}$, and $V_{\mathrm{LOW}}$ be the weighted vote totals and let
\begin{equation}
...
\centering
\scriptsize
\begin{tabular}{@{}lp{0.56\linewidth}l@{}}
\toprule
Parameter & Role & Evaluation default \\
\midrule
$N_{\mathrm{dvfs}}$ & Intents per per-GPU DVFS window & $16$ \\
$\lambda_q^{\alpha}$ & Criticality multiplier in slowdown estimate & $1.5$ \\
$\lambda_h^{\alpha}$ & Heavy-task multiplier in slowdown estimate & $1.0$ \\
$u_{\mathrm{force}}$ & Urgent-heavy mass force-high threshold & $0.15$ \\
$q_{\mathrm{force}}^{W}$ & Criticality-mass force-high threshold & $0.55$ \\
$b_{\mathrm{force}}^{W}$ & Average-backlog force-high threshold & $0.80$ \\
$\Omega_{\mathrm{mid}}$ & Minimum wait share for mid commit & $0.25$ \\
$u_{\mathrm{mid}}$ & Urgent-heavy mass limit for mid commit & $0.10$ \\
$q_{\mathrm{mid}}^{W}$ & Criticality-mass limit for mid commit & $0.45$ \\
$b_{\mathrm{mid}}^{W}$ & Average-backlog limit for mid commit & $0.60$ \\
$\lambda_{\mathrm{mid}}^{S}$ & Slack-consumption coefficient for mid & $0.35$ \\
$B_{\mathrm{mid}}$ & Additive mid performance budget & $0.20\,\mathrm{ms}$ \\
$p_{\mathrm{HIGH}}^{\max}$ & High-vote limit for considering low & $0.25$ \\
$h_{\mathrm{low}}^{\max}$ & Heavy-task fraction limit for low & $0.20$ \\
$b_{\mathrm{low}}^{\mathrm{forbid}}$ & Backlog threshold that forbids low & $0.70$ \\
$\Omega_{\mathrm{low}}$ & Minimum wait share for low commit & $0.40$ \\
$u_{\mathrm{low}}$ & Urgent-heavy mass limit for low commit & $0.0$ \\
$q_{\mathrm{low}}^{W}$ & Criticality-mass limit for low commit & $0.25$ \\
$b_{\mathrm{low}}^{W}$ & Average-backlog limit for low commit & $0.40$ \\
$\lambda_{\mathrm{low}}^{S}$ & Slack-consumption coefficient for low & $0.25$ \\
$B_{\mathrm{low}}$ & Additive low performance budget & $0.10\,\mathrm{ms}$ \\
$n_{\mathrm{mid}}$ & Consecutive eligible windows for mid & $2$ \\
$n_{\mathrm{low}}$ & Consecutive eligible windows for low & $3$ \\
\bottomrule
\end{tabular}
\caption{Window-level DVFS committer hyperparameters.}
\label{tab:dvfs-committer-hyperparameters}
\end{table}

The window is forced to high frequency when the mid level is unavailable, a cooldown is active, a critical override has been triggered, the urgent-heavy mass exceeds $u_{\mathrm{force}}$, the weighted criticality mass exceeds $q_{\mathrm{force}}^{W}$, or the average backlog exceeds $b_{\mathrm{force}}^{W}$. If none of these conditions holds, the mid level is eligible only when
\begin{equation}
\Omega\geq\Omega_{\mathrm{mid}},
\qquad
\mathrm{urgentHeavyMass}\leq u_{\mathrm{mid}},
\qquad
\bar{q}_{w}\leq q_{\mathrm{mid}}^{W},
\qquad
\overline{b}_{\mathrm{norm}}\leq b_{\mathrm{mid}}^{W},
\end{equation}
and
\begin{equation}
\sum_{t\in W_g}\Delta T_{\mathrm{MID}}(t)
\leq
\lambda_{\mathrm{mid}}^{S}\Sigma_{\mathrm{slack}}(g,W)+B_{\mathrm{mid}}.
\end{equation}
The low level is considered only when it is not disabled by global settings or guardrails, no cooldown is active, $p_{\mathrm{HIGH}}<p_{\mathrm{HIGH}}^{\max}$, the heavy-task fraction is below $h_{\mathrm{low}}^{\max}$, and the average backlog is below $b_{\mathrm{low}}^{\mathrm{forbid}}$. It is then eligible only when mid is eligible, the window contains enough intents, no recent slowdown guardrail is active, and
\begin{equation}
\Omega\geq\Omega_{\mathrm{low}},
\qquad
\mathrm{urgentHeavyMass}\leq u_{\mathrm{low}},
\qquad
\bar{q}_{w}\leq q_{\mathrm{low}}^{W},
\qquad
\overline{b}_{\mathrm{norm}}\leq b_{\mathrm{low}}^{W},
\end{equation}
with
\begin{equation}
\sum_{t\in W_g}\Delta T_{\mathrm{LOW}}(t)
\leq
\lambda_{\mathrm{low}}^{S}\Sigma_{\mathrm{slack}}(g,W)+B_{\mathrm{low}}.
\end{equation}

To avoid oscillation, the committer requires consecutive eligible windows before entering a lower level. The required counts are $n_{\mathrm{mid}}$ for $\mathrm{MID}$ and $n_{\mathrm{low}}$ for $\mathrm{LOW}$. If a forced-high condition is present, the committed level is $\mathrm{HIGH}$. If the previous level is $\mathrm{LOW}$ and low is no longer eligible, the committer returns to $\mathrm{HIGH}$. If the previous level is $\mathrm{MID}$ and mid is no longer eligible, it also returns to $\mathrm{HIGH}$. Otherwise, the committer enters $\mathrm{LOW}$ or $\mathrm{MID}$ only after the required consecutive eligibility count has been reached. Figure~\ref{fig:methods-dvfs-state-machine} summarizes the proposer--committer state machine.

\figMethodsDvfsStateMachine
...
After each per-GPU commit, the runtime updates exponential moving averages for two window-level measurements using coefficient $a_{\mathrm{ema}}$: the total window runtime and the busy-tail duration between the first and last task endpoints in the window. A slowdown indicator is raised when the current window runtime exceeds its moving average by more than $r_{\mathrm{win}}$ or when the busy-tail duration exceeds its moving average by more than $r_{\mathrm{tail}}$. Recent indicators are stored in a ring buffer of $n_{\mathrm{recent}}$ windows. If at least $n_{\mathrm{trigger}}$ of the recent windows indicate slowdown, the next windows are forced to $\mathrm{HIGH}$ frequency for a recovery interval. If the slowdown follows a $\mathrm{LOW}$ commit, $\mathrm{LOW}$ is disabled for a longer interval. This mechanism prevents an unsafe lower-frequency choice from persisting across multiple scheduling windows. Table~\ref{tab:dvfs-guardrail-hyperparameters} reports the default guardrail values used in the evaluation.
...
Parameter & Role & Evaluation default \\
\midrule
$r_{\mathrm{win}}$ & Window-runtime slowdown threshold & $2\%$ \\
$r_{\mathrm{tail}}$ & Busy-tail slowdown threshold & $3\%$ \\
$a_{\mathrm{ema}}$ & Exponential moving-average coefficient & $0.2$ \\
$n_{\mathrm{recent}}$ & Number of recent windows tracked & $4$ \\
$n_{\mathrm{trigger}}$ & Recent slowdown indicators needed for recovery & $2$ \\
\bottomrule
\end{tabular}
\caption{DVFS slowdown-guardrail hyperparameters.}
\label{tab:dvfs-guardrail-hyperparameters}
\end{table}
