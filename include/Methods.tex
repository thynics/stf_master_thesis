...
The DVFS layer is implemented as a conservative rule-based proposer and per-GPU window committer. It is separate from the placement bandit: placement chooses the GPU, and DVFS chooses the frequency level to use on that GPU. The DVFS layer is enabled by the rule-DVFS scheduler aliases listed in the implementation interface or by the corresponding DVFS enable environment variables.
...
The guardrail is not an additional frequency-optimization objective. Its role is to make the rule-based DVFS layer fail-safe. The proposer and committer decide from predicted task features, while the guardrail observes the realized behavior of recent windows and revokes lower-frequency operation when those decisions appear to increase runtime.
...
\texttt{CUDASTF\_SCHEDULE=}\newline\texttt{bandit\_scheduler\_rule\_dvfs} & Placement scheduler with the rule-based DVFS proposer and per-GPU committer enabled. \\
\texttt{CUDASTF\_SCHEDULE=}\newline\texttt{bandit\_scheduler\_rule\_dvfs\_}\newline\texttt{midrank}$\langle N\rangle$ & Rule-based DVFS path with a specified mid-frequency rank. \\
...
\texttt{CUDASTF\_DVFS\_}\newline\texttt{RULE\_ENABLE} or \texttt{CUDASTF\_DVFS\_ENABLE} & Environment-variable path for enabling the DVFS framework. \\
\hline\hline
\end{tabular}
\end{table}
