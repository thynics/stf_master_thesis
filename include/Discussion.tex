...
The CG mechanism diagnostic in Section~\ref{sec:results-cg-copy-latency}
adds an important nuance to this interpretation. Copy-byte reduction should not
be read as a direct decomposition of latency. In CG, the modeled copy-time
saving is only \(1.006\) ms, while the mean latency saving is \(1893.400\) ms.
The no-deviation control shows why this does not contradict the locality
explanation: when the same scheduler path is used but non-HEFT residual
placements are disabled, the result returns to the near-HEFT regime. The large
speedup appears only when a small and stable set of bounded residual choices is
enabled. Thus, copy bytes provide directional evidence that the accepted
placements are locality-oriented, while the larger latency effect comes from
the way those placements change downstream data residency, ready order, and
iterative execution.
