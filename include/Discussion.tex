...
the bandit scheduler improves geometric-mean kernel latency by
\(7.146\%\) across six benchmarks using mean latencies. The improvement is not
uniform. CG, Cholesky, and LU decomposition show
stable gains, whereas MiniWeather and Blocked N-body are
near neutral and FDTD is more fragile.
...
byte reductions: Cholesky reduces copy bytes by \(38.220\%\), and
LU decomposition reduces copy bytes by \(23.905\%\). The neutral
...
six final rows, WinDVFS reduces geometric-mean kernel energy by
\(2.904\%\) with a geometric-mean kernel-latency overhead of \(0.626\%\).
Four benchmarks show clear kernel-energy reductions, Cholesky shows
a smaller reduction with a larger latency trade-off, and
Blocked N-body remains near neutral.
...
time, the
Blocked N-body row shows why the guardrails are necessary: when the
...
MiniWeather and Blocked N-body are important precisely
...
FDTD results also show that benchmark-level noise can affect median
and mean conclusions differently. This is why the thesis treats
FDTD as a fragile locality case rather than as a primary proof point.
