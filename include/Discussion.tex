...
by \(9.99\%\) across the five reported benchmarks. The improvement is not
...
copy bytes by \(38.22\%\), LU decomposition reduces copy bytes by \(23.91\%\),
and CG reduces copy bytes by \(7.69\%\). MiniWeather keeps the same measured
...
five final rows, \WinDVFS{} reduces geometric-mean energy by \(5.32\%\) with a
geometric-mean latency overhead of \(0.48\%\). The geometric-mean EDP ratio is
0.951. FDTD, CG, MiniWeather, LU decomposition, and Cholesky all reduce EDP
...
a qualified positive answer. Energy decreases by \(5.32\%\) geometric mean,
while latency increases by only \(0.48\%\) geometric mean under the fixed
Bandit placement layer. The EDP ratio improves to 0.951. The result is
...
only \(-0.77\%\) to \(+1.44\%\) in latency across the five reported
benchmarks, with a geometric-mean static-replay delta of \(+0.17\%\). This
