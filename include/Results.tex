...
comparisons. Placement speedup is computed as HEFT kernel latency divided by
the scheduler kernel latency. Suite-level summaries use geometric means so
that each benchmark contributes multiplicatively rather than allowing one large
absolute runtime to dominate the aggregate.
...
Kernel power is the average NVML power over the same kernel window and is used
only to explain energy changes.

Whole-process energy is measured as an auxiliary diagnostic but is not used for
the main claims, because both the placement scheduler and the DVFS controller
act on the GPU kernel execution window. Energy--delay product is reported for
DVFS as \(E \times T\), and ED\(^2\)P as \(E \times T^2\), to show whether
energy savings still hold after accounting for runtime changes.
...
and commit or guardrail counters are used as DVFS evidence. These measurements
have different evidential roles: placement performance is the headline latency
result; locality rows are diagnostic attribution; DVFS rows are final
energy--performance measurements under the evaluated configuration; and the
overhead table is a diagnostic stress check rather than a production-overhead
estimate.
