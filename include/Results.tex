...
The measurements in this chapter have different evidential roles. The
placement headline is the strongest statistical result because it is computed
from five measured runs per scheduler and is accompanied by coefficient of
variation data. The locality-attribution rows are diagnostic and are used to
explain the direction of the placement effect rather than to recompute the
headline speedup. The DVFS rows are final kernel-window measurements and are
interpreted as energy--performance evidence under the evaluated configuration.
The overhead table is a single-run diagnostic snapshot and is not used as a
...
\item What overheads are introduced by compact placement logging and compact
WinDVFS control, and why are these diagnostic rows separated from the final
performance claims?
\end{enumerate}
...
appear on some workloads but not on all of them. The chapter ends with an
overhead diagnostic and an explicit statement of the boundaries of the claims,
rather than mixing diagnostic details into the main result tables.
...
example.
Because the FDTD latency delta is small relative to its run-to-run variation,
FDTD is used only as a weak positive placement case. The stronger placement
evidence comes from CG, Cholesky, and LU decomposition, where the effect sizes
are much larger than the reported CVs.
