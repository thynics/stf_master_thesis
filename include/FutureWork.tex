...
eight-GPU node and six benchmarks. Future experiments should repeat the
...
the locality wins remain stable when peer-copy bandwidth, copy-engine
contention, memory capacity, and kernel execution time change.
...
The \texttt{win\_dvfs} layer in this thesis focuses on guarded kernel-energy
reduction with bounded latency overhead. Future work could extend the
committer to optimize different objectives, such as EDP, ED$^2$P, or a
...
Instrumentation overhead should also be reduced. The overhead diagnostics show
that compact placement logging is usually small but that diagnostic
DVFS-control rows can be costly in some cases. A production-oriented
implementation should separate low-overhead counters used during normal
execution from detailed trace logging used only for debugging and thesis-style
attribution. This would keep the scheduler explainable without making detailed
logs part of the critical path.
